...
import uploaadOnCloudinary from "../config/cloudinary.js"
...
export const updateProfile=async(req,res)=>{
        try {
              ...
              if(req.files.profileImage){
                profileImage=await uploaadOnCloudinary(req.files.profileImage[0].path)
              }
              if(req.files.coverImage){
                coverImage=await uploaadOnCloudinary(req.files.coverImage[0].path)
              }
              let user=await User.findByIdAndUpdate(req.userId,{
                firstName,lastName,userName,headline,location,gender,skills,education,profileImage,coverImage
